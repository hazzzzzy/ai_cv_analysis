    ...
    useEffect(() => {
        const answered = new Set(
            messages
                ...
                .map(
                    (msg) =>
                        (msg.content_json as { question_id?: string } | null)
                            ?.question_id,
                )
        ...
        const pendingQuestion = messages
            .filter(
                (msg) => msg.role === "assistant" && msg.kind === "question",
            )
        ...
        try {
            ...
            if (resp.status === "completed") {
                ...
                setMessages((prev) =>
                    prev.filter(
                        (msg) =>
                            !(msg.role === "system" && msg.kind === "status"),
                    ),
                );
                setMessages((prev) => [
                    ...prev,
                    {
                        ...
                        kind: "final",
                        content: "最终分析已生成。",
                        content_json: resp.final_analysis as unknown as Record<
                            string,
                            unknown
                        >,
                    },
                ...
                setMessages((prev) => [
                    ...prev,
                    {
                        ...
                        content_json: resp.next_question as unknown as Record<
                            string,
                            unknown
                        >,
                    },
            ...
            setMessages((prev) =>
                prev.filter(
                    (msg) => !(msg.role === "system" && msg.kind === "status"),
                ),
            );
        } finally {
            setSubmitting(false);
                ...
                <aside className="panel-shell sidebar-scroll h-full">
                    ...
                    <div className="panel-section space-y-3 text-sm text-slate-600">
                        ...
                        <div>
                            简历 ID：
                            <span className="font-mono">
                                {session ? session.resume_id : "--"}
                            </span>
                        </div>
                        <Badge variant="outline">
                            {session
                                ? STATUS_LABEL[session.status] || "面试中"
                                : "未知"}
                        </Badge>
                        <Button
                            asChild
                                ...
                                {analysis && (
                                    <div className="space-y-6">
                                                ...
                                                {analysis.pain_points.map(
                                                    (item, index) => {
                                                        ...
                                                        return (
                                                            <div
                                                                key={`pain-${index}`}
                                                                className="rounded-2xl border border-slate-200/70 bg-slate-50/80 p-4 text-sm text-slate-700"
                                                            >
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="space-y-3">
                                                {analysis.improvement_directions.map(
                                                    (item, index) => {
                                                        const priority =
                                                            resolvePriority(
                                                                item.priority,
